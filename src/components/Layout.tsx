...
import { HomeIcon, SearchIcon, BriefcaseIcon, CoinsIcon, BellIcon, BookmarkIcon, UserIcon, SettingsIcon, HelpIcon, MessageIcon, PlusIcon } from './Icons'
        ...
        <header className="hidden lg:flex items-center gap-4 bg-white border-b border-[#e2e8f0] px-6 py-3 shadow-sm z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              />
              <span className="text-sm font-semibold text-[#0a1628]">{currentUser.name.split(' ')[0]}</span>
            </button>
          </div>
        </header>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[#e2e8f0] z-30 shadow-lg">
          <div className="flex items-center justify-around px-2 py-2">
            ...
            <MobileNavBtn
              ...
              onClick={() => navigate('messages')}
            />
            <MobileNavBtn icon={<UserIcon size={22} />} label="Perfil" active={['profile', 'settings', 'saved', 'coins', 'notifications'].includes(currentPage)} onClick={() => navigate('profile')} />
          </div>
        </nav>
      </div>
    </div>
